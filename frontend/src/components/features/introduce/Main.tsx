    ...
    >
      <Container maxWidth="md">
        ...
        <Typography variant="subtitle1" align="left" color="text.secondary" paragraph>
          　新潟県の特産品の一つである高級洋梨「ル レクチエ」は、形状や外観などに関する出荷規格に基づいて
          等級判定が行われています。現在、多くの場合この等級判定は生産者の目視により行われており、作業負担と
          判定における個人差の発生が課題となっています。新潟大学工学部山﨑研究室では、これまでル レクチエの
          外観品質の自動評価の研究を進め、特に深層学習を利用した外観汚損検出から等級判定まで行うシステムを
          提案しており、その過程でル レクチエの外観画像のデータセットを構築してきました。
          <br/>
          　農業分野での深層学習技術の適用が進められては来ているものの、データセット構築に専門家の知識が必要であることや
          作物ごとに学習に適した量のデータが必要であること等が、実用化へのボトルネックになっています。
          この課題の克服へ貢献するために、当研究室で構築したル レクチエのデータセットを公開し、農業分野での深層学習の実用化がより進展することを期待します。
        </Typography>
      </Container>
